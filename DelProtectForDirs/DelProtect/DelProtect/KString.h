...
class kstring final {
public:
	explicit kstring(const wchar_t* str = nullptr, ULONGLONG pool = POOL_FLAG_PAGED, ULONG tag = 0);
	kstring(const wchar_t* str, ULONG count, ULONGLONG pool = POOL_FLAG_PAGED, ULONG tag = 0);
	kstring(const kstring& other);
	explicit kstring(PCUNICODE_STRING str, ULONGLONG pool = POOL_FLAG_PAGED, ULONG tag = 0);
