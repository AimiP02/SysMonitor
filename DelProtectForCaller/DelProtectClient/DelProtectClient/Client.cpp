...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) {
		printf("Usage: %s [add] | [remove] | [clear] <file name>\n", argv[0]);
		return -1;
	}
	...
	switch (op) {
	case OPTIONS::ADD: {
		success = DeviceIoControl(hFile, IOCTL_DELPROTECT_ADD_EXE, (PVOID)fileName,
			static_cast<DWORD>(wcslen(fileName) + 1) * sizeof(WCHAR), nullptr, 0,
			&returned, nullptr);
		break;
	}
	case OPTIONS::REMOVE: {
		success = DeviceIoControl(hFile, IOCTL_DELPROTECT_REMOVE_EXE, (PVOID)fileName,
			static_cast<DWORD>(wcslen(fileName) + 1) * sizeof(WCHAR), nullptr, 0,
			&returned, nullptr);
		break;
	}
	case OPTIONS::CLEAR: {
		success = DeviceIoControl(hFile, IOCTL_DELPROTECT_CLEAR, nullptr,
			0, nullptr, 0,
			&returned, nullptr);
		break;
	}
